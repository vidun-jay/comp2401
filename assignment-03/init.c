...
/**
 * Initializes every field of the given arr parameter to default values
 * @param arr: array whose values to set
 * @param cap: capacity of the notebook
 */
void initNotebook (NotebookType *arr, int cap) {
  // the initial array capacity is set to the given parameter cap
  arr->capacity = cap;
  // the quantity of evidence currently stored in the new array is always zero
  arr->size = 0;
  // the elements array must be dynamically allocated to hold the number of elements indicated by the capacity
 ...
 * @param id: room id
 * @param room: name of room (living room, bathroom, etc.)
 * @param device: the device information was captured on
 * @param value: the reading from the device
 * @param timestamp: the time in seconds the info was recorded at
 * @return: EvidenceType ev
 */
void initEvidence(int id, char* room, char* device, float value, int timestamp, EvidenceType *ev) {
  ev->id = id;
  strcpy(ev->room, room);
  strcpy(ev->device, device);
  ev->value = value;
